...
def print_times(v, L):
    '''Print the number of milliseconds it takes for the linear_search(v, L) to
    run for list.index, basic linear search, the for loop linear search, and
    sentinel search.'''
    
    # Get the list.index's running time.
    t1 = time.time()
    L.index(v)
    t2 = time.time()
